...
class simple_social_interaction_buttons extends WP_Widget {

	// constructor
    function simple_social_interaction_buttons() {
        parent::WP_Widget(false, $name = __('Simple Social Interaction Buttons', 'simple_social_interaction_buttons') );
    }
...
function widget($args, $instance) {
   ...
   // Get $select value
	if( $twitteracc ) {
		include_once "includes/simple/twitter-follow.php";
			}	
	if( $facebooklike ) {
		include_once "includes/simple/facebook-like.php";
			}
	if( $googleplus ) {
		include_once "includes/simple/google-plus.php";
			}						
   echo '</div>';
   echo $after_widget;
}
...
// register widget
add_action('widgets_init', create_function('', 'return register_widget("simple_social_interaction_buttons");'));
